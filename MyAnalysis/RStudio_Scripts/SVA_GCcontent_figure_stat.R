...
xnonref <- read.table("GC_content_analysis/Results/RTEdb_SVAEF_GCresults20kb_26072019.txt", sep = "\t", header = T)
xref <- read.table("GC_content_analysis/Results/RefSVAAC_GCresults20kb_29052019.txt",sep = "\t", header = T)
...
p = ggplot(his_nonRef, aes(x=Var1,y=Pct, group = 1)) + 
  geom_line(aes(color="Non-Reference SVA"), size=1.5) +
  geom_line(data=his_Ref,aes(color="Reference SVA"), size=1.5)+
