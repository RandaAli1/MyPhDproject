# Regression analysis for the relationship between the density of L1s per chromosome and gene density

# This code conducts a simple linear regression analysis to determine whether there is 
# a significant relationship between the density of reference and non-reference L1s
...
nonref_L1density = Sheet1$Nonref_L1.Mb          # Density of non-reference L1s per chromosome
Ref_L1density = Sheet1$Ref_L1.Mb                # Density of reference L1s per chromosome


# Regression analysis for non-reference L1s density vs. gene density

# Create regression model for gene density vs. density of L1/chr:

linRegLine <- lm(nonref_L1density ~ GeneDensity)
summary(linRegLine)
...
## Regression analysis for reference L1s density vs. gene density

# Use same steps as non-reference L1s:

nrlinRegLine <- lm(Ref_L1density ~ GeneDensity)
...
## 
nrp = ggplot(new_df, aes(x=GeneDensity,y=Ref_L1density)) +
  geom_point(position=position_jitter(w=0.1,h=0)) +
  geom_text(aes(label=as.character(chrname)),hjust=0.5, vjust=-0.6, size = 3.5) +
  geom_smooth(method=lm, se = T) +
  #Add axis label
  xlab('Gene Density (genes/million bases)') +
  ylab('Reference L1s Density (L1s/million bases)') +
  geom_line(aes(y=lwr), color = "red", linetype = "dashed")+
  geom_line(aes(y=upr), color = "red", linetype = "dashed") 
