# Analysing the distribution of a control dataset based on 100 insertions in genic vs. intergenic regions.


# This code creates a control dataset of random insertions. The control dataset is used to subsample 5 sets of 
# 100 random insertions. Next, the frequency of random insertions interrupting an 
# intergenic, intronic, or exonic region is calculated for each set to create a background distribution.
...
bedtoolsRandomFun = function(functionstring="bedtools random",SampleSize,opt.string="") {
  ...
  command = paste(functionstring,"-g Enrichment_In_Functional_regions/Raw_data/
                chr_lengthbp.genome -n" ,SampleSize,opt.string,">",out,sep="\t")
  cat(command,"\n")
  try(system(command))
...
write.table(all_results, file = "Enrichment_In_Functional_regions/Results/Demo_RandomInsertionsIngeneregions.txt", sep="\t", row.names=F,col.names=T, quote = F)
