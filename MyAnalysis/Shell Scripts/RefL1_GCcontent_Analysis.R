...
# This code analysis the GC content of reference L1s in 20 kilobase (kb) windows.
...
locs <- read.delim("GC_content_analysis/Raw_data/L1PA2_L1PA5_refL1s_20kbwindow.bed", header=F, sep="\t")
...
GCcontent <- function(data, SampleSize){
  ...
  Gc.content <- (round(c((results$V13+results$V14)/(results$V12+results$V13+results$V14+results$V15)),3))*100
  return(Gc.content)
}
...
write.table(results, file = "GC_content_analysis/Results/RefL1PA2PA5_GCresults20kb_29052019.txt", sep="\t")
