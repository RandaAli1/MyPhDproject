# Analysing the distribution of a control dataset based on 100 elements
# in euchromatin regions

# This code creates a control dataset of random insertions. The control dataset is used to subsample 5 sets of 
# 100 random insertions. Next, the frequency of random insertions interrupting euchromatin regions in each of 
# the 127 H3K4me3 maps obtained from the roadmap website is calculated to create a background distribution.
...
bedTools.2in <- function(functionstring="bedtools intersect",d,opt.string="")
{
  ...
  command <- paste(functionstring,"-a",a.file,"-b Enrichment_in_Euchromatin_regions/Raw_data/Roadmap_sampleFile.bed -wa -wb | sort -k1,1V -k2,2n | cut -f13 | sort -k1,1V -k2,2n | uniq -c",opt.string,">",out,sep="\t")
  cat(command,"\n")
  try(system(command))
...
write.table(Results2, file = "Enrichment_in_Euchromatin_regions/Results/Demo_RandomH3K4Euchromatin.txt", sep="\t", row.names=F,col.names=T, quote = F)
