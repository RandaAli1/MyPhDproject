...
# This code plots the GC-content distribution of reference and non-reference SVAs
# in 2% bin. The GC-content distribution for reference and non-reference SVAs against 
...
xnonref <- read.table("GC_content_analysis/Results/RTEdb_AluY_GCresults20kb_26072019.txt", sep = "\t", header = T)
xref <- read.table("GC_content_analysis/Results/RefAluJ_GCresults20kb_29052019.txt",sep = "\t", header = T)
...
p = ggplot(his_nonRef, aes(x=Var1,y=Pct, group = 1)) + 
  geom_line(aes(color="Non-Reference Alus"), size=1.5) +
  geom_line(data=his_Ref,aes(color="Reference Alus"), size=1.5)+
